...
/**
 * A reconciliation pre-condition that is only met if Grpc GRPCRoute should be managed.
 * @author laurent
 */
public class MicrocksGRPCRouteInstallPrecondition implements Condition<HasMetadata, Microcks> {
   ...
   public boolean isMet(DependentResource<HasMetadata, Microcks> dependentResource, Microcks primary, Context<Microcks> context) {
      ...
      if (primary.getSpec().getMicrocks().getGrpcGatewayRoute() != null) {
         return primary.getSpec().getMicrocks().getGrpcGatewayRoute().isExpose();
      }
