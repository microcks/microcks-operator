...
/**
 * A reconciliation pre-condition that is only met if Grpc ingress should be managed.
 * @author laurent
 */
public class MicrocksGRPCIngressInstallPrecondition implements Condition<HasMetadata, Microcks> {
   ...
   public boolean isMet(DependentResource<HasMetadata, Microcks> dependentResource, Microcks primary, Context<Microcks> context) {
      ...
      if (primary.getSpec().getMicrocks().getGrpcIngress() != null) {
         return primary.getSpec().getMicrocks().getGrpcIngress().isExpose();
      }
