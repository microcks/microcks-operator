...
public class MongoDBDeploymentDependentResource extends CRUDKubernetesDependentResource<Deployment, Microcks>
   ...
   protected Deployment desired(Microcks microcks, Context<Microcks> context) {
      ...
      DeploymentBuilder builder = new DeploymentBuilder(deployment).editMetadata().withName(getDeploymentName(microcks))
            ...
            .withImage("centos/mongodb-34-centos7:latest").withResources(spec.getMongoDB().getResources()).addNewEnv()
            .withName("MONGODB_USER").withNewValueFrom().withNewSecretKeyRef()
            .withName(MongoDBSecretDependentResource.getSecretName(microcks))
            .withKey(MongoDBSecretDependentResource.MONGODB_USER_KEY).endSecretKeyRef().endValueFrom().endEnv()
            .addNewEnv().withName("MONGODB_PASSWORD").withNewValueFrom().withNewSecretKeyRef()
            .withName(MongoDBSecretDependentResource.getSecretName(microcks))
            .withKey(MongoDBSecretDependentResource.MONGODB_PASSWORD_KEY).endSecretKeyRef().endValueFrom().endEnv()
            .addNewEnv().withName("MONGODB_ADMIN_PASSWORD").withNewValueFrom().withNewSecretKeyRef()
            .withName(MongoDBSecretDependentResource.getSecretName(microcks))
            .withKey(MongoDBSecretDependentResource.MONGODB_ADMIN_PASSWORD_KEY).endSecretKeyRef().endValueFrom()
            .endEnv().addNewEnv().withName("MONGODB_DATABASE").withValue(microcksName).endEnv().endContainer().endSpec()
            .endTemplate().endSpec();

      if (spec.getKeycloak().isPersistent()) {
         builder.editSpec().editTemplate().editSpec().editFirstVolume().editOrNewPersistentVolumeClaim()
               .withClaimName(MongoDBPVCDependentResource.getPVCName(microcks)).endPersistentVolumeClaim().endVolume()
