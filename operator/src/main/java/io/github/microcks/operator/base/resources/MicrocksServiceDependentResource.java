...
public class MicrocksServiceDependentResource extends CRUDKubernetesDependentResource<Service, Microcks>
   ...
   protected Service desired(Microcks microcks, Context<Microcks> context) {
      ...
      ServiceBuilder builder = new ServiceBuilder().withNewMetadata().withName(getSecondaryResourceName(microcks))
            ...
            .addToLabels("container", "spring").addToLabels("group", "microcks").endMetadata().withNewSpec()
            .addToSelector("app", microcksName).addToSelector("container", "spring").addToSelector("group", "microcks")
            .addNewPort().withName("spring").withPort(8080).withProtocol("TCP").withTargetPort(new IntOrString(8080))
            .endPort().withSessionAffinity("None").withType("ClusterIP").endSpec();
